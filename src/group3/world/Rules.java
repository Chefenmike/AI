...
public class Rules {
	...
	/**
	 * Checks whether objectToMove is allowed ontop/inside destinationObject I
	 * reworked this method a bit so that it doesn't return true until all rules
	 * have been checked /Gustav.
	 * 
	 * @param objectToMove
	...
	private static boolean allowedMove(ObjectInWorld objectToMove,
		...
		if (objectToMove.getShape().equals(Shape.BALL)
				&& !destinationObject.getShape().equals(Shape.BOX)) {
			// balls must be in boxes (or on the floor)
			return false;
			// return (destinationObject.getShape().equals(Shape.BOX) ||
			// destinationObject.getShape().equals(Shape.FLOOR));
		}
		if (destinationObject.getShape().equals(Shape.BALL)) {
			// balls cannot support anything
			return false;
		}
		if (destinationObject.getSize().equals(Size.SMALL)
				&& objectToMove.getSize().equals(Size.LARGE)) {
			// small objects cannot support large objects
			return false;
		}
		if (destinationObject.getShape().equals(Shape.BOX)
				&& objectToMove.getSize().equals(destinationObject.getSize())) {
			if (objectToMove.getShape().equals(Shape.PYRAMID)
					|| objectToMove.getShape().equals(Shape.PLANK)) {
		...
		if (objectToMove.getShape().equals(Shape.BOX)) {
			...
			if (!objectToMove.getSize().equals(destinationObject.getSize())) {
				// both the box and the other object needs to be of the same
				// size
				return false;
			} else if (!(destinationObject.getShape().equals(Shape.TABLE)
					|| destinationObject.getShape().equals(Shape.PLANK) || destinationObject
					.getShape().equals(Shape.BRICK))) {
				// only tables, planks and bricks can support boxes
				return false;
			} else if (destinationObject.getShape().equals(Shape.BRICK)
					&& destinationObject.getSize().equals(Size.SMALL)) {
				// Small bricks cannot support any boxes
				return false;
			} else if (destinationObject.getShape().equals(Shape.BOX)) {
				// boxes cannot hold other boxes
				return false;
			}
		}
	 ...
	 *            the relation that objectToMove has to destinationObject
	 * @param destinationObject
	 * @return
	 */
	public static boolean allowedMove(ObjectInWorld objectToMove,
			RelativePosition relativePosition, ObjectInWorld destinationObject) {
		if (relativePosition.equals(RelativePosition.HOLDING)) {
			...
			// Objects cannot be placed under balls. A large object cannot be
			// placed under a small one.
			return !destinationObject.getShape().equals(Shape.BALL)
					&& !(objectToMove.getSize().equals(Size.LARGE) && destinationObject
							.getSize().equals(Size.SMALL));
		} else if (relativePosition.equals(RelativePosition.UNDER)) {
			// Objects cannot be placed under balls. A large object cannot be
			// placed under a small one.
			return !objectToMove.getShape().equals(Shape.BALL)
					&& !(destinationObject.getSize().equals(Size.LARGE) && objectToMove
							.getSize().equals(Size.SMALL));
